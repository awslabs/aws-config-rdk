...
class RulesTest:
    ...
    * **`rulenames`** (_str_): list of rule names to deploy
    ...
    def run(self):
        ...
        rules_list = []
        test_report = {"pytest_results": [], "cfn_guard_results": []}
        cwd = Path().absolute()
        ...
        for rule_path in rules_list:
            ...
            if runtime in (
                ...
                self.logger.info(
                    f"Skipping {rule_name} - The Custom Rule Runtime provided or Managed Rule is not supported for unit testing."
                )
    ...
    def _run_pytest(self, test_dir: Path):
        ...
        if len(results.errors) == 0 and len(results.failures) == 0:
            ...
            status = "FAILED"
        return {
            "rule_dir": test_dir.name,
            "status": status,
            "test_run": results.testsRun,
    ...
    def _run_cfn_guard_test(self, test_dir: Path):
        ...
        for test_path in test_dir.glob("**/*"):
            ...
            if (
                test_path.suffix in [".json", ".yaml", ".yml"]
                and test_path.name != "parameters.json"
            ):
                cfn_guard_runner = CfnGuardRunner(
                    rules_file=test_dir.joinpath("rule_code.guard"),
                    test_data=test_path,
                ...
                try:
                    ...
                    report.append(
                        {
                            "rule_dir": f"{test_dir.name}/{test_path.name}",
                            "status": "PASSED",
                            "test_run": results.count("Test Case #"),
                            "errors": [],
                            "failures": [],
                        }
                    )
    ...
    def _show_result(self, report_results: Dict[str, Any]):
        ...
        for result in report_results:
            ...
            self.logger.info(
                f"\tStatus: {result['status']} tests_run:{result['test_run']}"
            )
            if result["errors"]:
                exit_code = 1
