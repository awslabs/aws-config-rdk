...
def get_rule_name(rule_path: Path):
    ...
    try:
        ...
        raise RdkParametersInvalidError(
            f"Invalid parameters found in Parameters.RuleName in {rule_path}"
        )
    ...
    for path in rules_dir.absolute().glob("**/parameters.json"):
        if "build/" not in path.as_posix():
            ...
            #         raise RdkDuplicatedRuleNameError(rule_dir_paths)
                ...
                raise RdkNotSupportedError(
                    "Invalid Option: Specify Rule Name or RuleSet or empty for all."
                )
...
def validate(rule_dir: Path, parameters_json: dict):
    ...
    if (
        "Version" not in parameters_json
        and parameters_json["Version"] != latest_schema_version
    ):
        raise RdkParametersInvalidError(
            f"Error in {rule_dir}: Missing Version Key. The latest supported schema version is {latest_schema_version}"
        )
    if (
        "SourceIdentifier" not in parameters_json["Parameters"]
        and "SourceRuntime" not in parameters_json["Parameters"]
    ):
        raise RdkParametersInvalidError(
            f"Error in {rule_dir}: Missing Parameters.SourceIdentifier or Parameters.SourceRuntime is required"
        )
    if (
        "SourcePeriodic" not in parameters_json["Parameters"]
        and "SourceEvents" not in parameters_json["Parameters"]
    ):
        raise RdkParametersInvalidError(
            f"Error in {rule_dir}: Missing Parameters.SourcePeriodic or Parameters.SourceEvents is required"
        )
