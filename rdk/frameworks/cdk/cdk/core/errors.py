...
class RdkJsonInvalidError(Exception):
    ...
    def __init__(self, rule_dir):
        message = f"Failed to decode JSON in parameters file in {rule_dir}"
...
class RdkJsonLoadFailure(Exception):
    ...
    def __init__(self, rule_dir):
        message = f"Error loading parameters file in {rule_dir}"
...
class RdkDuplicatedRuleNameError(Exception):
    ...
    def __init__(self, rule_paths):
        message = f"Found duplicated rule name in the following paths: {rule_paths}"
