...
PARALLEL_COMMAND_THROTTLE_PERIOD = (
    2  # 2 seconds, used in running commands in parallel over multiple regions
)
...
def get_command_parser():
    ...
    parser.add_argument(
        "-p", "--profile", help="[optional] indicate which Profile to use."
    )
    parser.add_argument(
        "-k", "--access-key-id", help="[optional] Access Key ID to use."
    )
    parser.add_argument(
        "-s", "--secret-access-key", help="[optional] Secret Access Key to use."
    )
    parser.add_argument(
        "-r", "--region", help="Select the region to run the command in."
    )
    parser.add_argument(
        "-f",
        "--region-file",
...
def get_rule_parser(is_required, command):
    ...
    parser.add_argument(
        "rulename", metavar="<rulename>", help="Rule name to create/modify"
    )
    ...
    parser.add_argument(
        "--optional-parameters", help="[optional] JSON for optional Config parameters."
    )
    parser.add_argument(
        "--tags",
...
def get_deployment_parser(ForceArgument=False, Command="deploy"):
    ...
    parser.add_argument(
        "-s", "--rulesets", required=False, help="comma-delimited list of RuleSet names"
    )
    parser.add_argument(
        "-f",
        "--functions-only",
...
def get_deployment_organization_parser(
    ForceArgument=False, Command="deploy-organization"
):
    direction = "to"
    if Command == "undeploy":
        direction = "from"
...
def get_export_parser(ForceArgument=False, Command="export"):
    ...
    parser.add_argument(
        "-f", "--format", required=True, help="Export Format", choices=["terraform"]
    )
    parser.add_argument(
        "-g",
        "--generated-lambda-layer",
...
def get_test_parser(command):
    parser = argparse.ArgumentParser(
        prog="rdk " + command, description="Used to run tests on your Config Rule code."
    )
    parser.add_argument(
        "rulename",
    ...
    parser.add_argument(
        "--test-ci-json", "-j", help="[optional] JSON for test CI for testing."
    )
    parser.add_argument(
        "--test-ci-types", "-t", help="[optional] CI type to use for testing."
    )
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="[optional] Enable full log output"
    )
    parser.add_argument(
        "-s",
        "--rulesets",
...
def get_logs_parser():
    ...
    parser.add_argument(
        "rulename", metavar="<rulename>", help="Rule whose logs will be displayed"
    )
    parser.add_argument(
        "-f",
        "--follow",
...
def get_rulesets_parser():
    ...
    parser.add_argument(
        "rulename", nargs="?", help="Name of Rule to be added or removed"
    )
    return parser
...
def parse_region_file(args):
    ...
    try:
        ...
        raise SyntaxError(
            f"Error reading regions: {region_set} in file: {args.region_file}"
        )
...
class rdk:
    ...
    def init(self):
        ...
        if self.args.config_bucket_exists_in_another_account:
            print(
                f"[{my_session.region_name}]: Skipping Config Bucket check due to command line args"
            )
        ...
        if self.args.generate_lambda_layer:
            lambda_layer_version = self.__get_existing_lambda_layer(
                my_session, layer_name=self.args.custom_layer_name
            )
            if lambda_layer_version:
                print(
                    f"[{my_session.region_name}]: Found Version: "
            ...
            self.__create_new_lambda_layer(
                my_session, layer_name=self.args.custom_layer_name
            )
            lambda_layer_version = self.__get_existing_lambda_layer(
                my_session, layer_name=self.args.custom_layer_name
        ...
        if len(recorders["ConfigurationRecorders"]) > 0:
            ...
            print(
                f"[{my_session.region_name}]: Found Config Recorder: "
        ...
        if len(delivery_channels["DeliveryChannels"]) > 0:
            ...
            config_bucket_name = delivery_channels["DeliveryChannels"][0][
                "s3BucketName"
            ]
        ...
        if not config_bucket_exists:
            ...
            for bucket in response["Buckets"]:
                if bucket["Name"] == config_bucket_name:
                    print(
                        f"[{my_session.region_name}]: Found Bucket: "
                        + config_bucket_name
                    )
                    config_bucket_exists = True
                    bucket_exists = True
            ...
            if not bucket_exists:
                print(
                    f"[{my_session.region_name}]: Creating Config bucket "
                    + config_bucket_name
                )
        ...
        if not config_role_arn:
            ...
            if not role_exists:
                ...
                assume_role_policy_template = open(
                    os.path.join(
                        path.dirname(__file__), "template", assume_role_policy_file
                    ),
                    "r",
                ).read()
                assume_role_policy = json.loads(
                    assume_role_policy_template.replace(
                        "${PARTITIONURL}", partition_url
                    )
                )
            ...
            policy_template = open(
                os.path.join(
                    path.dirname(__file__), "template", delivery_permission_policy_file
                ),
                "r",
            ).read()
            delivery_permissions_policy = policy_template.replace(
        ...
        if not control_tower and not config_recorder_exists:
            ...
            if not delivery_channel_exists:
                ...
                print(
                    f"[{my_session.region_name}]: Creating delivery channel to bucket "
                    + config_bucket_name
                )
            ...
            my_config.start_configuration_recorder(
        ...
        # create code bucket
        code_bucket_name = (
            code_bucket_prefix + account_id + "-" + my_session.region_name
        )
        response = my_s3.list_buckets()
        bucket_exists = False
        for bucket in response["Buckets"]:
            if bucket["Name"] == code_bucket_name:
                bucket_exists = True
                print(
                    f"[{my_session.region_name}]: Found code bucket: "
                    + code_bucket_name
                )
        ...
        if not bucket_exists:
            if self.args.skip_code_bucket_creation:
                print(
                    f"[{my_session.region_name}]: Skipping Code Bucket creation due to command line args"
                )
            else:
                print(
                    f"[{my_session.region_name}]: Creating Code bucket "
                    + code_bucket_name
                )
    ...
    def clean(self):
        ...
        if len(recorders["ConfigurationRecorders"]) > 0:
            ...
            try:
                ...
                my_config.stop_configuration_recorder(
                    ConfigurationRecorderName=recorders["ConfigurationRecorders"][0][
                        "name"
                    ]
                )
        ...
        try:
            ...
            try:
                role_policy_results = iam_client.list_role_policies(
                    RoleName=config_role_name
                )
                for policy_name in role_policy_results["PolicyNames"]:
                    iam_client.delete_role_policy(
                        RoleName=config_role_name, PolicyName=policy_name
                    )
                ...
                for policy in role_policy_results["AttachedPolicies"]:
                    iam_client.detach_role_policy(
                        RoleName=config_role_name, PolicyArn=policy["PolicyArn"]
                    )
        ...
        if len(delivery_channels["DeliveryChannels"]) > 0:
            for delivery_channel in delivery_channels["DeliveryChannels"]:
                config_bucket_names.append(
                    delivery_channels["DeliveryChannels"][0]["s3BucketName"]
                )
                try:
                    ...
                    print(
                        "Error encountered trying to delete Delivery Channel: " + str(e)
                    )
                ...
                if self.args.runtime == "java8":
                    ...
                    for line in f:
                        if self.args.runtime in [
                            ...
                            print(
                                line.replace("<%RuleName%>", self.args.rulename), end=""
                            )
                    f.close()
                    ...
                    if os.path.exists(src):
                        dst = os.path.join(
                            ...
                            self.args.rulename
                            + "_test"
                            + extension_mapping[self.args.runtime],
                        )
                        shutil.copyfile(src, dst)
                        f = fileinput.input(files=dst, inplace=True)
                        for line in f:
    ...
    def modify(self):
        ...
        if not self.args.remediation_action and "Remediation" in old_params:
            ...
            if execution_controls:
                ...
                self.args.remediation_error_rate_percent = ssm_controls.get(
                    "ErrorPercentage", ""
                )
            self.args.remediation_parameters = (
                json.dumps(params["Parameters"]) if params.get("Parameters") else None
            )
            self.args.auto_remediation_retry_attempts = params.get(
                "MaximumAutomaticAttempts", ""
            )
            self.args.auto_remediation_retry_time = params.get(
                "RetryAttemptSeconds", ""
            )
        ...
        print(
            "Modified Rule '"
            + self.args.rulename
            + "'.  Use the `deploy` command to push your changes to AWS."
        )
    ...
    def undeploy(self):
        ...
        if not self.args.force:
            ...
            while not confirmation:
                my_input = input(
                    "Delete specified Rules and Lambda Functions from your AWS Account? (y/N): "
                )
                if my_input.lower() == "y":
                    confirmation = True
                if my_input.lower() == "n" or my_input == "":
        ...
        for rule_name in rule_names:
            try:
                cfn_client.delete_stack(
                    StackName=self.__get_stack_name_from_rule_name(rule_name)
                )
        ...
        print(
            f"[{my_session.region_name}]: Rule removal initiated. Waiting for Stack Deletion to complete."
        )
        ...
        print(
            f"[{my_session.region_name}]: Rule removal complete, but local files have been preserved."
        )
    ...
    def undeploy_organization(self):
        ...
        if not self.args.force:
            ...
            while not confirmation:
                my_input = input(
                    "Delete specified Rules and Lambda Functions from your Organization? (y/N): "
                )
                if my_input.lower() == "y":
                    confirmation = True
                if my_input.lower() == "n" or my_input == "":
        ...
        print(
            f"[{my_session.region_name}]: To re-deploy, use the 'deploy-organization' command."
        )
    ...
    def deploy(self):
        ...
        if self.args.functions_only:
            ...
            for rule_name in rule_names:
                ...
                if "SourceIdentifier" in rule_params:
                    print(
                        f"[{my_session.region_name}]: Skipping code packaging for Managed Rule."
                    )
                else:
            ...
            try:
                ...
                print(
                    f"[{my_session.region_name}]: Updating CloudFormation Stack for Lambda functions."
                )
                try:
                    cfn_args = {
                    ...
                    if e.response["Error"]["Code"] == "ValidationError":
                        if "No updates are to be performed." in str(e):
                            ...
                            print(
                                f"[{my_session.region_name}]: No changes to Config Rule configurations."
                            )
                        else:
                ...
                for rule_name in rule_names:
                    ...
                    if "SourceIdentifier" in rule_params:
                        print(
                            f"[{my_session.region_name}]: Skipping Lambda upload for Managed Rule."
                        )
                        continue
                ...
                print(
                    f"[{my_session.region_name}]: Creating CloudFormation Stack for Lambda Functions."
                )
        ...
        for rule_name in rule_names:
            ...
            if "InputParameters" in rule_params:
                combined_input_parameters.update(
                    json.loads(rule_params["InputParameters"])
                )
            ...
            if "SourceIdentifier" in rule_params:
                ...
                if "Remediation" in rule_params:
                    print(
                        f"[{my_session.region_name}]: Build The CFN Template with Remediation Settings"
                    )
                    ...
                    remediation = self.__create_remediation_cloudformation_block(
                        rule_params["Remediation"]
                    )
                    ...
                    if "SSMAutomation" in rule_params:
                        ...
                        print(
                            f"[{my_session.region_name}]: Building SSM Automation Section"
                        )
                        ...
                        json_body["Resources"][
                            self.__get_alphanumeric_rule_name(
                                rule_name + "RemediationAction"
                            )
                        ] = ssm_automation
                        if "IAM" in rule_params["SSMAutomation"]:
                            print(
                                f"[{my_session.region_name}]: Lets Build IAM Role and Policy"
                            )
                            # TODO Check For IAM Settings
                            json_body["Resources"]["Remediation"]["Properties"][
                                "Parameters"
                            ...
                            print(
                                f"[{my_session.region_name}]: Build Supporting SSM Resources"
                            )
                            resource_depends_on = [
                                "rdkConfigRule",
                                self.__get_alphanumeric_rule_name(
                            ...
                            json_body["Resources"]["Remediation"][
                                "DependsOn"
                            ] = resource_depends_on
                            json_body["Resources"]["Remediation"]["Properties"][
                                "TargetId"
                            ] = {
                                "Ref": self.__get_alphanumeric_rule_name(
                                    rule_name + "RemediationAction"
                                )
                            }
                    ...
                    try:
                        ...
                        # If we've gotten here, stack exists and we should update it.
                        print(
                            f"[{my_session.region_name}]: Updating CloudFormation Stack for "
                            + rule_name
                        )
                        try:
                            cfn_args = {
                            ...
                            if e.response["Error"]["Code"] == "ValidationError":
                                if "No updates are to be performed." in str(e):
                                    # No changes made to Config rule definition, so CloudFormation won't do anything.
                                    print(
                                        f"[{my_session.region_name}]: No changes to Config Rule."
                                    )
                                else:
                                    # Something unexpected has gone wrong.  Emit an error and bail.
                                    print(f"[{my_session.region_name}]: {e}")
                        ...
                        # If we're in the exception, the stack does not exist and we should create it.
                        print(
                            f"[{my_session.region_name}]: Creating CloudFormation Stack for "
                            + rule_name
                        )
                    ...
                    # deploy config rule
                    cfn_body = os.path.join(
                        path.dirname(__file__), "template", "configManagedRule.json"
                    )
            ...
            if self.args.lambda_role_arn:
                print(
                    f"[{my_session.region_name}]: Existing IAM Role provided: "
                    + self.args.lambda_role_arn
                )
                lambdaRoleArn = self.args.lambda_role_arn
            elif self.args.lambda_role_name:
                print(
                    f"[{my_session.region_name}]: Building IAM Role ARN from Name: "
                    + self.args.lambda_role_name
                )
            ...
            if self.args.boundary_policy_arn:
                print(
                    f"[{my_session.region_name}]: Boundary Policy provided: "
                    + self.args.boundary_policy_arn
                )
            ...
            if layers:
                my_params.append(
                    {"ParameterKey": "Layers", "ParameterValue": ",".join(layers)}
                )
            ...
            cfn_body = os.path.join(
                path.dirname(__file__), "template", "configRule.json"
            )
            template_body = open(cfn_body, "r").read()
            json_body = json.loads(template_body)
            ...
            if "SSMAutomation" in rule_params:
                ...
                if "IAM" in rule_params["SSMAutomation"]:
                    ...
                    # TODO Check For IAM Settings
                    json_body["Resources"]["Remediation"]["Properties"]["Parameters"][
                ...
                try:
                    ...
                    if e.response["Error"]["Code"] == "ValidationError":
                        if "No updates are to be performed." in str(e):
                            ...
                            # Something unexpected has gone wrong.  Emit an error and bail.
                            print(
                                f"[{my_session.region_name}]: Validation Error on CFN\n"
                            )
                            print(
                                f"[{my_session.region_name}]: "
                                + json.dumps(cfn_args)
                                + "\n"
                            )
                            print(f"[{my_session.region_name}]: {e}\n")
    ...
    def deploy_organization(self):
        ...
        for rule_name in rule_names:
            ...
            # create json of CFN template
            cfn_body = os.path.join(
                path.dirname(__file__), "template", "configRuleOrganization.json"
            )
            template_body = open(cfn_body, "r").read()
            json_body = json.loads(template_body)
    ...
    def export(self):
        ...
        for rule_name in rule_names:
            ...
            params_file_path = os.path.join(
                os.getcwd(), rules_dir, rule_name, rule_name.lower() + ".tfvars.json"
            )
            ...
            tf_file_path = os.path.join(
                os.getcwd(), rules_dir, rule_name, rule_name.lower() + "_rule.tf"
            )
            ...
            variables_file_path = os.path.join(
                os.getcwd(), rules_dir, rule_name, rule_name.lower() + "_variables.tf"
            )
        ...
        for rule_name in rule_names:
            ...
            if rule_params["SourceRuntime"] not in (
                ...
                print(
                    "Skipping "
                    + rule_name
                    + " - Runtime not supported for local testing."
                )
                continue
            ...
            if args.verbose == True:
                results = unittest.TextTestRunner(buffer=False, verbosity=2).run(
                    self.__create_test_suite(test_dir)
                )
            else:
                results = unittest.TextTestRunner(buffer=True, verbosity=2).run(
                    self.__create_test_suite(test_dir)
                )
        ...
        for rule_name in rule_names:
            ...
            for my_ci in my_cis:
                ...
                test_event = json.load(
                    open(
                        os.path.join(
                            path.dirname(__file__), "template", event_template_filename
                        ),
                        "r",
                    ),
                    strict=False,
    ...
    def logs(self):
        ...
        try:
            log_streams = cw_logs.describe_log_streams(
                ...
                limit=int(
                    self.args.number
                ),  # This is the worst-case scenario if there is only one event per stream
            )
            ...
            my_events = self.__get_log_events(
                cw_logs, log_streams, int(self.args.number)
            )
    ...
    def rulesets(self):
        ...
        if self.args.subcommand in ["add", "remove"] and (
            not self.args.ruleset or not self.args.rulename
        ):
            print(
                "You must specify a ruleset name and a rule for the `add` and `remove` commands."
            )
            return 1
    ...
    def create_terraform_template(self):
        self.args = get_create_rule_template_parser().parse_args(
            self.args.command_args, self.args
        )
    ...
    def create_rule_template(self):
        ...
        parameters["LambdaAccountId"][
            "Description"
        ] = "Account ID that contains Lambda functions for Config Rules."
        ...
        if not self.args.rules_only:
            ...
            if self.args.config_role_arn:
                resources["ConfigurationRecorder"]["Properties"][
                    "RoleARN"
                ] = self.args.config_role_arn
        ...
        for rule_name in rule_names:
            ...
            for input_param in input_params:
                ...
                cfn_param["Description"] = (
                    "Pass-through to required Input Parameter "
                    + input_param
                    + " for Config Rule "
                    + rule_name
                )
            ...
            if "OptionalParameters" in params:
                ...
                for optional_param in optional_params:
                    cfn_param = {}
                    cfn_param["Description"] = (
                        "Pass-through to optional Input Parameter "
                        + optional_param
                        + " for Config Rule "
                        + rule_name
                    )
                    ...
                    param_name = (
                        self.__get_alphanumeric_rule_name(rule_name) + optional_param
                    )
                    ...
                    conditions[param_name] = {
                        "Fn::Not": [{"Fn::Equals": ["", {"Ref": param_name}]}]
                    }
            ...
            if "InputParameters" in params:
                for required_param in json.loads(params["InputParameters"]):
                    cfn_param_name = (
                        self.__get_alphanumeric_rule_name(rule_name) + required_param
                    )
                    properties["InputParameters"][required_param] = {
                        "Ref": cfn_param_name
                    }
            ...
            config_rule_resource_name = (
                self.__get_alphanumeric_rule_name(rule_name) + "ConfigRule"
            )
            ...
            if "Remediation" in params:
                ...
                if "SSMAutomation" in params:
                    ssm_automation = self.__create_automation_cloudformation_block(
                        params["SSMAutomation"], rule_name
                    )
                    # AWS needs to build the SSM before the Config Rule
                    remediation["DependsOn"].append(
                        self.__get_alphanumeric_rule_name(
                            rule_name + "RemediationAction"
                        )
                    )
                    # Add JSON Reference to SSM Document { "Ref" : "MyEC2Instance" }
                    remediation["Properties"]["TargetId"] = {
                        "Ref": self.__get_alphanumeric_rule_name(rule_name)
                        + "RemediationAction"
                    }
                    ...
                    if "IAM" in params["SSMAutomation"]:
                        ...
                        remediation["Properties"]["Parameters"]["AutomationAssumeRole"][
                            "StaticValue"
                        ]["Values"] = [
                            {
                                "Fn::GetAtt": [
                ...
                resources[
                    self.__get_alphanumeric_rule_name(rule_name) + "Remediation"
                ] = remediation
        ...
        if script_for_tag:
            print(
                "Found tags on config rules. Cloudformation do not support tagging config rule at the moment"
            )
            ...
            if self.args.tag_config_rules_script:
                ...
                print(
                    "you can use flag [--tag-config-rules-script <file path> ] to output the script"
                )

    def create_region_set(self):
        self.args = get_create_region_set_parser().parse_args(
            self.args.command_args, self.args
        )
    ...
    def __create_java_rule(self):
        src = os.path.join(
            path.dirname(__file__), "template", "runtime", "java8", "src"
        )
        dst = os.path.join(os.getcwd(), rules_dir, self.args.rulename, "src")
        shutil.copytree(src, dst)

        src = os.path.join(
            path.dirname(__file__), "template", "runtime", "java8", "jars"
        )
        dst = os.path.join(os.getcwd(), rules_dir, self.args.rulename, "jars")
        shutil.copytree(src, dst)

        src = os.path.join(
            path.dirname(__file__), "template", "runtime", "java8", "build.gradle"
        )
    ...
    def __print_log_event(self, event):
        ...
        for line in message_lines:
            ...
            formatted_lines.append(
                "\n".join(
                    line[i : i + line_wrap] for i in range(0, len(line), line_wrap)
                )
            )
    ...
    def __get_rule_list_for_command(self, Command="deploy"):
        ...
        if self.args.all:
                ...
                if os.path.isdir(obj_path) and not obj_name == "rdk":
                    for file_name in os.listdir(obj_path):
                        if obj_name not in rule_names:
                            if os.path.exists(
                                os.path.join(obj_path, "parameters.json")
                            ):
                                ...
                                if os.path.exists(
                                    ...
                                    rule_names.append(obj_name)
                                if os.path.exists(
                                    os.path.join(obj_path, "RuleCode.cs")
                                ):
                                    rule_names.append(obj_name)
            ...
            print(
                'Invalid Option: Specify Rule Name or RuleSet. Run "rdk %s -h" for more info.'
                % (Command)
            )
            sys.exit(1)
    ...
    def __get_rule_parameters(self, rule_name):
        params_file_path = os.path.join(
            os.getcwd(), rules_dir, rule_name, parameter_file_name
        )
    ...
    def __parse_rule_args(self, is_required):
        self.args = get_rule_parser(is_required, self.args.command).parse_args(
            self.args.command_args, self.args
        )
        ...
        if (
            is_required
            and not self.args.resource_types
            and not self.args.maximum_frequency
        ):
            print(
                "You must specify either a resource type trigger or a maximum frequency."
            )
            sys.exit(1)
        ...
        if self.args.optional_parameters:
            try:
                optional_params_dict = json.loads(
                    self.args.optional_parameters, strict=False
                )
            except Exception as e:
    ...
    def __parse_test_args(self):
        self.args = get_test_parser(self.args.command).parse_args(
            self.args.command_args, self.args
        )
    ...
    def __parse_deploy_args(self, ForceArgument=False):
        self.args = get_deployment_parser(ForceArgument).parse_args(
            self.args.command_args, self.args
        )
        ...
        if self.args.stack_name and not self.args.functions_only:
            print(
                "--stack-name can only be specified when using the --functions-only feature."
            )
            sys.exit(1)
        ...
        if self.args.lambda_layers:
            ...
            if layer_count > 5:
                ...
                sys.exit(1)
            if (
                self.args.rdklib_layer_arn
                or self.args.generated_lambda_layer
                and layer_count > 4
            ):
                print(
                    "Because you have selected a 'lib' runtime You may only specify 4 additional Lambda Layers."
                )
                sys.exit(1)

        # RDKLib version and RDKLib Layer ARN/Generated RDKLib Layer are mutually exclusive.
        if "rdk_lib_version" in self.args and (
            self.args.rdklib_layer_arn or self.args.generated_lambda_layer
        ):
            print(
        ...
        if self.args.rdklib_layer_arn and self.args.generated_lambda_layer:
            print(
                "Specify EITHER an RDK Lib Layer ARN OR the generated lambda layer flag."
            )
            sys.exit(1)
    ...
    def __parse_deploy_organization_args(self, ForceArgument=False):
        self.args = get_deployment_organization_parser(ForceArgument).parse_args(
            self.args.command_args, self.args
        )
    ...
    def __parse_export_args(self, ForceArgument=False):
        self.args = get_export_parser(ForceArgument).parse_args(
            self.args.command_args, self.args
        )
    ...
    def __populate_params(self):
        ...
        if (
            ...
            print(
                "Remediation Flags detected but no remediation action (--remediation-action) set"
            )
    ...
    def __generate_remediation_params(self):
        ...
        if self.args.remediation_concurrent_execution_percent:
            ssm_controls[
                "ConcurrentExecutionRatePercentage"
            ] = self.args.remediation_concurrent_execution_percent
        ...
        if self.args.auto_remediation_retry_attempts:
            params[
                "MaximumAutomaticAttempts"
            ] = self.args.auto_remediation_retry_attempts
    ...
    def __write_params_file(self, rulename, parameters, tags):
        ...
        params_file_path = os.path.join(
            os.getcwd(), rules_dir, rulename, parameter_file_name
        )
        parameters_file = open(params_file_path, "w")
    ...
    def __wait_for_cfn_stack(self, cfn_client, stackname):
        ...
        while in_progress:
            ...
            if all_deleted:
                in_progress = False
                print(
                    f"[{my_session.region_name}]: CloudFormation stack operation complete."
                )
                continue
            else:
                if "FAILED" in active_stack["StackStatus"]:
                    in_progress = False
                    print(
                        f"[{my_session.region_name}]: CloudFormation stack operation Failed for "
                        + stackname
                        + "."
                    )
                    if "StackStatusReason" in active_stack:
                        print(
                            f"[{my_session.region_name}]: Reason: "
                            + active_stack["StackStatusReason"]
                        )
                elif active_stack["StackStatus"] == "ROLLBACK_COMPLETE":
                    in_progress = False
                    print(
                        f"[{my_session.region_name}]: CloudFormation stack operation Rolled Back for "
                        + stackname
                        + "."
                    )
                    if "StackStatusReason" in active_stack:
                    ...
                    print(
                        f"[{my_session.region_name}]: Waiting for CloudFormation stack operation to complete..."
                    )
                    time.sleep(5)
    ...
    def __get_test_CIs(self, rulename):
        ...
        if self.args.test_ci_types:
            ...
            tests_path = os.path.join(
                os.getcwd(), rules_dir, rulename, test_ci_filename
            )
            if os.path.exists(tests_path):
                print(
                    "\tTesting with CI's provided in test_ci.json file. NOT YET IMPLEMENTED"
                )  # TODO
    ...
    def __get_lambda_arn_for_stack(self, stack_name):
        ...
        if my_lambda_arn == "NOTFOUND":
            print(
                f"[{my_session.region_name}]: Could not read CloudFormation stack output to find Lambda function."
            )
            sys.exit(1)
    ...
    def __get_lambda_name(self, rule_name, params):
        if "CustomLambdaName" in params:
            ...
            lambda_name = "RDK-Rule-Function-" + self.__get_stack_name_from_rule_name(
                rule_name
            )
            if len(lambda_name) > 64:
                print(
    ...
    def __upload_function_code(
        self, rule_name, params, account_id, my_session, code_bucket_name
    ):
        if params["SourceRuntime"] == "java8":
            # Do java build and package.
    ...
    def __create_automation_iam_cloudformation_block(self, ssm_automation, rule_name):
        ...
        ssm_automation_iam_role = {
            "Type": "AWS::IAM::Role",
            "Properties": {
                "Description": "IAM Role to Support Config Remediation for "
                + rule_name,
        ...
        ssm_automation_iam_policy = {
            ...
            "Properties": {
                ...
                "PolicyName": {
                    "Fn::Sub": "" + rule_name + "-Remediation-Policy-${AWS::Region}"
                },
                "Roles": [
    ...
    def __create_function_cloudformation_template(self):
        ...
        parameters["SourceBucket"][
            "Description"
        ] = "Name of the S3 bucket that you have stored the rule zip files in."
        ...
        for rule_name in rule_names:
            ...
            lambda_permissions["Properties"] = {
                "FunctionName": {
                    "Fn::GetAtt": [alphanum_rule_name + "LambdaFunction", "Arn"]
                },
    ...
    def __tag_config_rule(self, rule_name, cfn_tags, my_session):
        ...
        config_arn = config_client.describe_config_rules(ConfigRuleNames=[rule_name])[
            "ConfigRules"
        ][0]["ConfigRuleArn"]
    ...
    def __get_lambda_layers(self, my_session, args, params):
        ...
        if "SourceRuntime" in params:
            if params["SourceRuntime"] in [
                ...
                if (
                    hasattr(args, "generated_lambda_layer")
                    and args.generated_lambda_layer
                ):
                    lambda_layer_version = self.__get_existing_lambda_layer(
                    ...
                    rdklib_arn = RDKLIB_ARN_STRING.format(
                        region=my_session.region_name, version=rdk_lib_version
                    )
    ...
    def __create_new_lambda_layer(self, my_session, layer_name="rdklib-layer"):
        ...
        if layer_name == "rdklib-layer":
            successful_return = self.__create_new_lambda_layer_serverless_repo(
                my_session
            )
    ...
    def __create_new_lambda_layer_serverless_repo(self, my_session):
        try:
            ...
            print(
                f"[{my_session.region_name}]: Creating change set to deploy rdklib-layer"
            )
            ...
            if code == -1:
                print(
                    f"[{my_session.region_name}]: Error creating change set, attempting to use manual deployment"
                )
                raise ClientError()
            print(
                f"[{my_session.region_name}]: Executing change set to deploy rdklib-layer"
            )
    ...
    def __create_new_lambda_layer_locally(self, my_session, layer_name="rdklib-layer"):
        ...
        shell_command = (
            "pip3 install --target python boto3 botocore rdk rdklib future mock"
        )
    ...
    def __init__(self, ci_type):
        ...
        try:
            self.ci_json = json.load(
                open(
                    os.path.join(
                        path.dirname(__file__), "template", example_ci_dir, ci_file
                    ),
                    "r",
                )
            )
