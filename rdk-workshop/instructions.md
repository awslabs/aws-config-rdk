...
```
rdk create MFA_ENABLED_RULE --runtime python3.14 --resource-types AWS::IAM::User
```

  This will create a Rule backed by a Lambda function with that uses the python3.14 runtime and is triggered by resource changes to IAM users.  In your development environment this will create a directory named MFA_ENABLED_RULE, and populate it with Lambda boilerplate code (`MFA_ENABLED_RULE.py`), a base set of unit tests (`MFA_ENABLED_RULE_test.py`), and a metadata file that RDK uses to keep track of Rule configuration (`parameters.json`).  
