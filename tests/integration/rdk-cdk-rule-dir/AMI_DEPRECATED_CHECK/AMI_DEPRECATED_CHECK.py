    ...
    def evaluate_ami(self, ec2_client, ami_id):
        if not ami_id:
            ...
            return (
                ...
                f"Image {ami_id} is either unshared or deleted",
            )
        try:
            ...
            deprecation_time = datetime.strptime(
                image["DeprecationTime"], "%Y-%m-%dT%H:%M:%S.%fZ"
            )
            ...
            print(
                f"Exception checking {ami_id}, assuming deprecated/unshared/deleted: {e}"
            )
            return (
                ComplianceType.NON_COMPLIANT,
                f"Error checking {ami_id}, assuming noncompliant",
            )
