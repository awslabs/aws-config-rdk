...
class ComplianceTest(unittest.TestCase):
    ...
    instance_response = {
        "Reservations": [
            {
                "Groups": [],
                "Instances": [{"ImageId": "ami-abcd1234", "InstanceId": "i-abcd1234"}],
            }
        ]
    }
    ...
    asg_launch_config = {
        "AutoScalingGroups": [
            {"AutoScalingGroupName": "test-asg", "LaunchConfigurationName": "test-lc"}
        ]
    }
    ...
    launch_config = {
        "LaunchConfigurations": [
            {"LaunchConfigurationName": "test-lc", "ImageId": "ami-abcd1234"}
        ]
    }
    ...
    def test_evaluate_asg_mixed_instances_launch_template_compliant(self):
        ASG_CLIENT_MOCK.describe_auto_scaling_groups.return_value = (
            self.asg_mixed_instances
        )
        EC2_CLIENT_MOCK.describe_launch_template_versions.return_value = (
        ...
        launch_template_version = self.launch_template_versions[
            "LaunchTemplateVersions"
        ][0]
        response_expected = [
            Evaluation(
    ...
    def test_evaluate_noncompliant_asg_launch_template_missing_ami(self):
        ASG_CLIENT_MOCK.describe_auto_scaling_groups.return_value = (
            self.asg_launch_template
        )
        ASG_CLIENT_MOCK.describe_launch_template_versions.return_value = (
            self.launch_template_versions
        )
        EC2_CLIENT_MOCK.describe_images.return_value = self.missing_ami_response
        response = RULE.evaluate_periodic({}, CLIENT_FACTORY, {"mode": "ASG"})
